...
export default function App() {
  ...
  const hasAny = salesItems || workItems
  const effectiveSales = salesItems || []
  const effectiveWork = workItems || []

  function reset() { setSalesItems(null); setWorkItems(null); setError(null) }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 max-w-5xl mx-auto">
      ...
      {hasAny && (
        <>
          <SummaryCards salesItems={effectiveSales} workItems={effectiveWork} />
          {workItems && <WorkSection items={workItems} />}
          {salesItems && <ChannelBreakdown items={salesItems} />}
          {salesItems && <SalesTable items={salesItems} />}
