...
export default function CashRegister({
  ...
  // Аренда / гаражи — режут ТОЛЬКО зарплату дня, кассы не касаются
  ...
  // Расчётный остаток — движение кассы (аренда/гаражи НЕ входят)
  // берём расходыОпер (без помещений), автоАренду не вычитаем
  const расчётный = остатокВчера + реализация + работа + приходОзон + приходЯндекс
                  - расходыОпер - зарплата - закупка - возвраты - вычет
  ...
  // Зарплата дня = маржа + работа - операционные расходы - аренда/гаражи
  const зарплатаДня = валоваяПрибыль - расходыОпер - аренда
