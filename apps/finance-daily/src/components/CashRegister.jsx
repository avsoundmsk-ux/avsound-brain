...
export default function CashRegister({
  ...
  // Аренда/гаражи из файла Расходы — реальная оплата, вычитается из КАССЫ (не из зп)
  ...
  // Авто-аренда 5000/день — «откладываем на аренду», вычитается из ЗП
  const автоАренда = 5000 * rentDays

  // Расчётный остаток — движение кассы (полные расходы, включая оплату аренды/гаражей; авто-отложение не трогает кассу)
  ...
  // Зарплата дня = маржа + работа - операционные расходы - авто-аренда (5000/день)
  const зарплатаДня = валоваяПрибыль - расходыОпер - автоАренда
  ...
  function buildPayload() {
    ...
    return {
      ...
      summary: {
        ...
        работа, валоваяПрибыль, расходы, зарплата, закупка, возвраты, вычет, аренда: автоАренда, помещенияРасх,
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 flex items-end gap-4 flex-wrap">
        ...
        <div className="pb-2 text-sm text-gray-600">
          <b>{rentDays}</b> {rentDays === 1 ? 'день' : rentDays < 5 ? 'дня' : 'дней'} · авто-аренда <b>{fmt(автоАренда)}</b> (из зп)
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="mt-4 space-y-0.5 border-t border-gray-100 pt-3">
            ...
            {помещенияРасх > 0 && <Row label="Аренда/гаражи опл." value={помещенияРасх} sign="−" color="text-gray-500" />}
