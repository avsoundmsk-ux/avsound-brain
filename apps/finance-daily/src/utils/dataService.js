...
export function getMonthlyReport() {
  ...
  days.forEach(d => {
    ...
    // Пересчёт налету по текущей формуле: маржа + работа − расходы(опер.) − авто-аренда
    // (оплата аренды/гаражей из файла и вычет — только касса, прибыль не режут)
    {
      const расходыОпер = (s.расходы || 0) - (s.помещенияРасх || 0)
      m.прибыльДня += (s.маржа||0) + (s.работа||0) - расходыОпер - (s.аренда || RENT_PER_DAY)
    }
...
export function getChartData(limitDays = 60) {
  ...
  return days.map(d => {
    ...
    return {
      ...
      // Пересчёт: маржа + работа − расходы(опер.) − авто-аренда
      прибыль:     (s.маржа||0) + (s.работа||0) - ((s.расходы||0) - (s.помещенияРасх||0)) - (s.аренда || RENT_PER_DAY),
      закупка:     s.закупка       || 0,
    ...
    try {
      ...
      fetched.push({
        ...
        summary: {
          ...
          помещенияРасх: ds.помещения || 0,
          прибыльДня:    (ds.маржа||0) + (ds.работа||0) - ((ds.расходы||0) - (ds.помещения||0)) - (ds.аренда||RENT_PER_DAY),
        },
        ...
        возвраты: (detail.returns        || []).map(r => ({ comment: r.описание, сумма: r.сумма })),
